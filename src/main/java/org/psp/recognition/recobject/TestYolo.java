...
public class TestYolo extends RecObject {
    ...
    public void init() {
        if (AppProperties.getInstance().getProperties().get("detection").get("TestYolo").equals("on")) {
            isOn = true;
    ...
    protected void preProcess() {
        ...
        ArrayList<String> classNames = getResource(YOLO_CLASS_NAMES).getLines();

        Net net = Dnn.readNetFromDarknet(getResource(YOLO_CFG_FILE).getAbsolutePath(), getResource(YOLO_DARKNET_MODEL).getAbsolutePath());

        DetectionModel detectionModel = new DetectionModel(net);
